...
IRC.prototype._initHook = function () {
  ...
  self.on('i.sendMsg.o.sendMsg', function () {
    self.say(arguments[2].dest, arguments[2].msg);
  });

  self.on('i.command.o.command', function (fullEvent, event, command) {
    self.command(command);
  });
  
  self.on('i.join.o.join', function (fullEvent, event, channel) {
    self.join(channel);
  });
  
  self.on('i.part.o.part', function (fullEvent, event, channel) {
    self.part(channel);
  });

  self.on('i.exit.o.exit', function (fullEvent, event, message) {
    self.exit(message);
  });
}
...
IRC.prototype.command = function (command, payload) {
  this.ircClient.send(command, payload.replace('\n', '').replace('\r', ''));
