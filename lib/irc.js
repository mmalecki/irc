  ...
  self.on('hook::ready', function () {
    ...
    self.ircClient = new ircModule.Client(
      ...
      self.nick,
      {
        ...
        showErrors: self.config.get('showErrors'),
...
IRC.prototype._initPrompt = function () {
  prompt.start();
  prompt.pause();
  prompt.message = this.config.get('promptMsg').red.bold;
  prompt.delimiter = this.config.get('promptDelim').blue.bold;
}

IRC.prototype._initIRC = function () {
  var self = this;
  self.ircClient.on('motd', function (motd) {
    self.ircClient.say('nickserv', 'identify ' + self.config.get('password'));
    self.emit('ircConnected', {channels: self.channels, ircNick: self.nick});
    if (self.config.get('useShell')) {
      self._initPrompt();
      self.shell(); 
    }
