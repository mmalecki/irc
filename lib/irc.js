...
var IRC = exports.IRC = function(options){
  var defaultOpts = {
    ...
    nick: 'refactorbot',
    channels: ['#kohai', '#nodetestsu', '#nodebombrange'],
    showErrors: true,
    userName: 'hookio',
    realName: 'Hook.io IRC Client',
    idCheck: true,
    promptMsg: 'Hook.io-IRC'.red.bold,
    promptDelim: '>'.blue.bold,
    useShell: false
  };
...
IRC.prototype._initIRC = function () {
  ...
  self.ircClient.on('motd', function (motd) {
    self.emit('o.ircConnect', {channels: self.channels, ircNick: self.nick});
  ...
  self.ircClient.on('notice', function (nick, to, text) {
    if (!nick) {
      console.log('Server Message: '.green, to.green, text.green);
...
IRC.prototype._initHook = function () {
  ...
  self.on('i.sendMsg.o.sendMsg', function () {
    self.say(arguments[1].dest, arguments[1].msg);
  });

  self.on('i.command.o.command', function (event, data) {
    self.command(data);
  });
  
  self.on('i.join.o.join', function (event, channel) {
    self.join(channel);
  });
  
  self.on('i.part.o.part', function (event, channel) {
    self.part(channel);
  });

  self.on('i.exit.o.exit', function (event, message) {
    self.exit(message);
  });
}
