...
IRC.prototype._initIRC = function () {
  ...
  self.ircClient.on('join',function (channel, nick) {
    if (nick !== self.nick) {
      self.emit('irc::userJoined', { channel: channel, nick: nick });
    }
  });  

  self.ircClient.on('part',function (channel, nick, reason) {
    if (nick !== self.nick) {
      self.emit('irc::userParted', { channel: channel, nick: nick, reason: reason });
    }
  });  
...
IRC.prototype._initHook = function () {
  ...
  self.on('**::irc::msg', function (data) {
    console.log('got message');
    self.say(data);
  });

  self.on('**::irc::command', function (data) {
    self.command(data);
  });
  
  self.on('**::irc::join', function (data) {
    self.join(data);
  });
  
  self.on('**::irc::part', function (data) {
    self.part(data);
  });

  self.on('**::irc::exit', function (data) {
    self.exit(data);
  });
}

IRC.prototype.say = function (msg) {
  var self = this;
  ...
  this.emit('irc::msg', 'Message has been sent!');
}
